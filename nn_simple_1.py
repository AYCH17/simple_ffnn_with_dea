...
def dea(fobj,train_set,targets, bounds, mut=0.8, crossp=0.7, popsize=100, its=1000):
    nn = NeuralNet()
    ...
    for i in range(len(pop)) : 
        nn.set_weights_and_bias(torch.from_numpy(pop_denorm[i]))
        fitness = np.append(fitness,fobj(train_set,targets))
    ...
    for i in range(its):
        for j in range(popsize):
            ...
            ##
            nn.set_weights_and_bias(torch.from_numpy(trial_denorm))
            ##
            f = fobj(train_set,targets)
            if f < fitness[j]:
                fitness[j] = f
                pop[j] = trial
...
nn1 =NeuralNet()
best_weights_bias , score = dea(nn1.fast_fonction_objective,train_set,train_targets, [(0,1)] * 13002)

print(best_weights_bias,score)

nn1.set_weights_and_bias(torch.from_numpy(best_weights_bias))
#%%
test(test_dataloader,nn1)
